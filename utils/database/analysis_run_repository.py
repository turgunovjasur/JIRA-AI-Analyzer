...
Struktura ataylab checker_run_repository bilan bir xil — checker keyin shu generic
jadvalga oson ko'chadi (FAZA: checker migratsiya).
"""
...
def ensure_analysis_run_tables(conn) -> None:
    ...
    cursor.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_analysis_agent_runs_unique ON analysis_agent_runs(run_id, agent_key)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_analysis_runs_state ON analysis_runs(module_key, run_state, updated_at)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_analysis_events_run_id ON analysis_run_events(run_id, id)"
    )
    conn.commit()
    ...
    *,
    ...
    row = _execute(
        ...
        INSERT INTO analysis_runs (
            run_id, module_key, task_key, company_id, user_id, source, execution_mode,
...
def update_analysis_run(conn, run_id: str, **fields: Any) -> dict[str, Any] | None:
    ...
    _execute(
        conn,
        f"UPDATE analysis_runs SET {columns} WHERE run_id = ?",
        params,
    )
    conn.commit()
    return get_analysis_run(conn, run_id)
...
def update_analysis_agent_run(
    ...
    **fields: Any,
    ...
    if not fields:
        return get_analysis_agent_run(conn, run_id, agent_key)
    now = _now_iso()
    field_map = dict(fields)
    field_map["updated_at"] = now
    ...
    _execute(
        conn,
        f"UPDATE analysis_agent_runs SET {columns} WHERE run_id = ? AND agent_key = ?",
        params,
    )
    conn.commit()
    return get_analysis_agent_run(conn, run_id, agent_key)
...
def save_analysis_run_final_result(
    ...
    *,
    ...
    return update_analysis_run(
        ...
        status_message="Run yakunlandi" if run_state == "completed" else "Run xato yoki block holatida tugadi",
...
def get_analysis_run(conn, run_id: str) -> dict[str, Any] | None:
    row = _execute(
        conn,
        "SELECT * FROM analysis_runs WHERE run_id = ? LIMIT 1",
        [run_id],
    ).fetchone()
    if not row:
        return None
...
def get_analysis_agent_run(conn, run_id: str, agent_key: str) -> dict[str, Any] | None:
    row = _execute(
        conn,
        """
        SELECT * FROM analysis_agent_runs
...
def list_analysis_run_events(conn, run_id: str) -> list[dict[str, Any]]:
    rows = _execute(
        conn,
        """
        SELECT * FROM analysis_run_events
        WHERE run_id = ?
        ORDER BY id ASC
        """,
        [run_id],
...
def build_analysis_run_snapshot(conn, run_id: str) -> dict[str, Any] | None:
    run = get_analysis_run(conn, run_id)
    if not run:
        return None
    return {
        **run,
        "agent_runs": list_analysis_agent_runs(conn, run_id),
        "run_events": list_analysis_run_events(conn, run_id),
