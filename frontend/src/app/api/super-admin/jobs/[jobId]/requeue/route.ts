...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    const payload = await backendRequest<{ success: boolean; job: Record<string, unknown> }>(
      `/api/admin/jobs/${jobId}/requeue`,
      { method: "POST" },
    );
    ...
    const message =
      routeError instanceof Error ? routeError.message : "Job requeue bo'lmadi.";
